...
public class ItemCanisterLiquidOxygen3 extends ItemCanisterTierEmpty implements IItemOxygenSupply {
    private static HashMap<ItemStack, Integer> craftingvalues = new HashMap();
    protected IIcon[] icons = new IIcon[7];
    private int mPreCapacity;

    public ItemCanisterLiquidOxygen3(String assetName) {
    ...
    public static void saveDamage(ItemStack itemstack, int damage) {
        ItemCanisterLiquidOxygen3.craftingvalues.put(itemstack, Integer.valueOf(damage));
    }
    ...
    public String getUnlocalizedName(ItemStack itemStack) {
        ...
        if (itemStack.getItemDamage() == 1) {
            return "item.canister.LOX3.full";
        }

        return "item.canister.LOX3.partial";
    }
    ...
    public ItemStack getContainerItem(ItemStack itemstack) {
        Integer saved = ItemCanisterLiquidOxygen3.craftingvalues.get(itemstack);
        if (saved != null) {
            if (saved < this.mPreCapacity + 1) {
                ItemCanisterLiquidOxygen3.craftingvalues.remove(itemstack);
